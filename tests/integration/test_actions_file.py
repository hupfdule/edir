    ...
    def assertListsEqual(self, actual, expected):
        ...
        for elm in actual:
            if not elm in expected:
                ...
                Expected elements: {sorted(expected)}
                ''')
...
class TestReadActionsFile(unittest.TestCase, CustomAssertions):
    ...
    def tearDown(self):
        ...
        # FIXME: This is unclean. Path should not be used statically inside edir
        edir.Path.paths = []
    ...
    def test_order_actions_file(self):
        """
        Test that the order of the actions file will be corrected to correctly execute all actions.

        That means:
        1. copy
        2. rename

        Deletion is not supported at the same time as copy or rename in
        action files as this is not possible to achieve with a normal edir
        run.
        """
        # - preparation
        ...
        self.assertDirContainsExactlyFiles(
            testdir,
            {
              'file1copy':    "file 1 content",
              'file1renamed': "file 1 content",
              'file2copy':    "file 2 content",
              'file2renamed': "file 2 content",
              'file3':        "file 3 content",
              'file3copy':    "file 3 content",
...
def change_tmp_dir(tempdir):
    global orig_tmp_dir
    orig_tmp_dir = {}
    orig_tmp_dir['TMP']    = os.getenv('TMP')
    orig_tmp_dir['TEMP']   = os.getenv('TeMP')
    orig_tmp_dir['TMPDIR'] = os.getenv('TMPDIR')
...
def restore_tmp_dir():
    ...
    for var in ['TMP', 'TEMP', 'TMPDIR']:
        if orig_tmp_dir[var] is None:
            os.environ.pop(var, None)
        else:
            os.environ[var]      = orig_tmp_dir[var]
    orig_tmp_dir = None
